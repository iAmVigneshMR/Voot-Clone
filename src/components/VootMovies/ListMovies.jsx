...
const ListMovies = () => {
    ...
    useEffect(() => {
        let fetchData = async () => {
            let fetMovies = await firebase.database().ref('voot-video');
            //firebase event for fetching data (value = event for fetching data)
            fetMovies.on("value", vootMovies => {
                console.log((vootMovies));
                console.log(vootMovies.val()); //val() used to Extracts a JavaScript value from a DataSnapshot
                  let VOOT_MOVIES = [];
